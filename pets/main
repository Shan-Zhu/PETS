...
import pets.data_reporting as data_reporting
import pets.io_utils as IO
import pets.sim as sim
import pets.utils as utils
...
def run_simulation(ndD_s, ndD_e, tScale, outdir):
    ...
    simulation = sim.SimPETS(ndD_s, ndD_e, tScale)
...
def main(paramfile, keepArchive=True):
    ...
    if commit_hash != "":
        ...
        with open(os.path.join(outdir, 'run_info.txt'), 'w') as fo:
            ...
            print("$ python[3] petsrun.py input_params_system.cfg", file=fo)
